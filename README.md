...
- [✨ 核心功能亮點](#-核心功能亮點)
- [📱 應用介面預覽](#-應用介面預覽)
- [🏆 第一階段開發成果總結 (Phase 1 Summary)](#-第一階段開發成果總結-phase-1-summary)
- [🏗️ 技術架構](#️-技術架構)
- [📂 專案目錄結構](#-專案目錄結構)
- [🚀 快速開始](#-快速開始)
  - [環境需求](#環境需求)
...
* 📅 **智慧排課與即時檢視**：視覺化週課表排程、支援 1~14 節自訂節次、單雙週/自訂週次設定、開學與結束日期動態計算、週末顯示開關、支援批次新增課程。
...
* 📈 **百分制與多元 GPA 運算**：支援百分制、4.3 制與 4.0 制等計算標準，即時試算各學期平均分數與歷年累計 GPA。
* 💰 **生活記帳、月曆視圖與多帳戶管理**：支援「列表視圖」與「月曆視圖 (Calendar View)」切換，自訂管理多種支付帳戶（現金、銀行帳戶、LINE Pay、信用卡等，支援排序、編輯與預設），搭配月度預算警戒線。
* 🔔 **全方位通知與自訂偏好**：整合課程提醒、記帳警示、學業審查提醒等多維度開關。
...
## 🏆 第一階段開發成果總結 (Phase 1 Summary)

第一階段（v1.0.0）已圓滿完成所有核心基礎設施與大學生日常管理模組的開發與驗證：

### 🎯 階段完成指標 (Milestone Achievements)

```text
[第一階段核心交付清單]
├── 📅 課表與排程系統  ── 100% (週課表、考勤打卡、課程筆記、單雙週、批量排課)
├── 🎓 學業與畢業審查  ── 100% (百分制 GPA 試算、子分類必選修門檻、門檻清單)
├── 💰 記帳與資產管理  ── 100% (月曆視圖、自訂多支付帳戶、月度預算警戒)
├── 🔔 通知偏好與提醒  ── 100% (課堂提醒、記帳警示、學業審查提醒)
├── ☁️ 雲端同步與隱私  ── 100% (Firebase 雙向安全同步、防覆蓋拉取、JSON 匯入匯出)
└── 🏗️ 架構與品質工程  ── 100% (MVVM + Clean Architecture、Robolectric & Roborazzi 測試覆蓋)
```

| 模組領域 | 交付成果與技術重點 |
| :--- | :--- |
| **智慧週課表模組** | 視覺化網格週課表、自訂節次（1~14節）、學期起始/結束日期換算、單雙週過濾、課程考勤點名 (出席/遲到/曠課/請假統計)、課程筆記與重點管理、批量排課。 |
| **學業與畢業審查模組** | 百分制與 4.3/4.0 GPA 試算引擎、校共同/院核心/系專業（基礎/核心/專業模組）等全分類學分稽核、必修與選修細項門檻設定、外語/專題/證照非學分門檻檢核。 |
| **個人記帳與財務模組** | 支援列表與月曆 (Calendar) 雙視圖切換、自訂多支付帳戶管理 (支援自訂名稱/色彩/排序/預設帳戶)、收支分類標籤、月預算動態進度條與超支警示。 |
| **通知與偏好設定** | 整合課程提醒、記帳提醒與學業審查推播設定，支援週末課表顯示切換。 |
| **帳號、隱私與同步** | 100% 離線 Room 本機資料庫、Firebase Auth (Google & Email)、Firestore 雲端雙向同步（含自訂帳戶同步與防本機空覆蓋安全機制）、自訂使用者頭像、JSON 匯出/匯入。 |
| **工程與品質架構** | 採用 Modern Android Architecture (MVVM + UDF)、Jetpack Compose Material 3 Edge-to-Edge 介面、Coroutines + StateFlow 響應式架構、JUnit4 + Robolectric + Roborazzi 截圖測試。 |
...
│   │   │   │   │   │   ├── expense/              # 個人記帳、月曆視圖、多帳戶管理、月預算控制
│   │   │   │   │   │   └── settings/             # 帳號設定、通知偏好、雲端同步、JSON 檔案匯入/匯出
...
* **視覺化課表視圖**：支援週一至週日、第 1 至 14 節網格排課，自訂色彩標籤與週末顯示開關。
* **學期與週次管理**：支援多學期動態切換、開學與結束日期精準設定、單雙週過濾與批次新增課程。
...
* **雙檢視模式**：支援「列表視圖」與「月曆視圖 (Calendar View)」，點選日期快速查看每日收支明細。
* **自訂多支付帳戶管理 (Multi-Account)**：
  * 支援自訂新增/編輯/刪除/排序支付帳戶（如現金、各銀行帳戶、LINE Pay、街口等）。
  * 支援預設帳戶設定與雲端跨裝置同步。
* **快速記帳與預算警戒**：提供餐飲、交通、娛樂、學習、住宿等豐富標籤；設定每月總預算，以動態進度條即時警示花費進度防範超支。
...
* **個人化通知設定**：支援自訂課程提醒（如上課前 15 分鐘）、每日記帳提醒、學業審查提醒。
* **頭像與個人檔案**：支援使用者自訂頭像保存與暱稱展示。
...
* [ ] 🤖 **AI 課堂助理 (Gemini API 整合)**：課程筆記重點摘要、學習建議與期末準備指南。
* [ ] 📸 **智慧 OCR / PDF 匯入**：拍照或上傳學校課表/選課清單自動帶入。
